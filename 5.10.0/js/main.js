...
const continentColors = d3.scaleOrdinal(d3.schemeDark2.slice(2));
...
d3.json("data/data.json").then((data) => {
	...
	//filter the data
	filtered = data.map(year => {
		return year.countries.filter(country => (country.income && country.life_exp)).map(country => {
